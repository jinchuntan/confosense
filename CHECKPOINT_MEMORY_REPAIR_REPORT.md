...
| Preparation/windows/identity/read/validation region | 76.584 s (sum of corresponding phases) | 50.132 s (enclosing region) |
...
The two authorized additional folds completed sequentially with exit status 0 in fresh directories, retaining the same scientific settings. Both independent audits and normal zero-fit resumes passed. Their separate resource measurements and results are recorded in the [three-fold benchmark report](BDG2_THREEFOLD_BENCHMARK_REPORT.md). Full-study readiness remains false.
