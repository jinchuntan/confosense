...
def verify_frozen(manifest):
    ...
    assert len(manifest['units'])==52 and [tuple(u['key']) for u in manifest['units']]==KEYS
    assert sha(REVIEW/'frozen_scope.csv')==manifest['scope_hash']
...
def progress(engine,manifest):
    ...
    engine.state.update(completed_units=len(done),planned_units=52,completed_keys=done,pending_keys=pending,
    ...
    engine.save()
    body='# Matched fold-2 multiseed batch progress and restart\n\n'+f'Updated UTC: {now()}. Status: **{engine.state["status"]}**. Validated **{len(done)}/52 new paired units**; {3*len(done)} point / {6*len(done)} interval cells.\n\n'
    body+='The atomic [progress ledger](../../smart_building_conformal/outputs/matched_forecasting005/fold2_multiseed_batch_v1/progress.json) and [append-only attempts](../../smart_building_conformal/outputs/matched_forecasting005/fold2_multiseed_batch_v1/attempts.jsonl) retain exact commands, process identities, actual exits and receipt paths. A running process is not a successful exit.\n\n'
...
def accept(unit,engine):
    ...
    group_task=engine.state['tasks'][s+'/groups']
    group_check=read(Path(group_task['argv'][group_task['argv'].index('--out')+1])/'validation.json')
    assert group_check['passed'] and group_check['actual_seed_verified'] and group_check['persistence_seed42_alias_exact']
    ...
    with exclusive_lock(BACKUP/'coordinator.lock'):
        ...
        try:
            ...
            for unit in manifest['units']:
                ...
                print(f'VALIDATED {unit["order"]}/52 {s}',flush=True)
                if unit['order'] in (13,26,39):
                    seed=unit['key'][3]
                    engine.phase(f'seed{seed}/analysis',lambda folder:[PYTHON,'-B',str(REVIEW/'analyze.py'),'seed','--out',str(BATCH/f'seed{seed}_summary_v1')])
                    engine.phase(f'publication/seed{seed}',lambda folder:[PYTHON,'-B',str(REVIEW/'publish.py'),'--label',f'seed{seed}'])
            ...
            print('ALL 52 UNITS AND FIVE-SEED REPORTS COMPLETE',flush=True)
