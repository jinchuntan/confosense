...
def md(f):return f.to_markdown(index=False,floatfmt='.6g')
...
def main(version):
    ...
    with Operations(forbid=True):
        ...
        for ds in ['pleia_energy','rico']:
            ...
            v=read(batch/(ds+'_validation')/'validation.json');r=read(batch/(ds+'_resume.json'))
        ...
        worker=pd.DataFrame([dict(task=k,actual_exit=r['exit_code'],seconds=r['seconds'],receipt=r['logger_receipt']) for k,r in state['tasks'].items()]);csv(out/'cli_costs_and_exits.csv',worker)
