...
def main(version):
    ...
    with Operations(forbid=True):
        ...
        run_seconds=float(worker[worker.task=='pleia_energy/run'].seconds.iloc[0]);validation_seconds=float(worker[worker.task=='pleia_energy/validate'].seconds.iloc[0])
        text+=f'The complete synthetic PLEIA run took {run_seconds:.3f} seconds and its successful independent validation took {validation_seconds:.3f} seconds. Scaling elapsed time by 68/4 original contexts gives approximately **{run_seconds*17/60:.1f} minutes for execution and {validation_seconds*17/60:.1f} minutes for validation**, before a measured real-fit allowance. This elapsed-time calculation includes the synthetic feature reconstruction, checkpoint hashing and event-table overhead omitted by the narrower context-stage timing above. It assumes similar alias incidence and per-context cost. The real feature count, 4,954-row initialization pool and larger estimators can change it substantially; it is a planning extrapolation, not a runtime guarantee or a measured real C result.\n\n'
