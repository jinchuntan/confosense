...
def main(out):
    ...
    protocol=read(DESIGN/'frozen_protocol.json');validation=read(BATCH/'validation_v2/validation.json');resume=read(BATCH/'completed_resume_v1.json')
    ...
    for action in ('run','validate','resume'):
        m=read(BATCH/(action+'_worker_resources_v2.json' if action=='validate' else action+'_worker_resources_v1.json'));assert m['actual_exit']==0
        measures.append(dict(action=action,actual_exit=0,started_utc=m['started_utc'],ended_utc=m['ended_utc'],pid=m['pid'],**m['resources']))
    failed=read(BATCH/'validate_worker_resources_v1.json')
    assert failed['actual_exit']==1
    pd.DataFrame(measures+[dict(action='validate_failed_v1',actual_exit=1,started_utc=failed['started_utc'],ended_utc=failed['ended_utc'],pid=failed['pid'],**failed['resources'])]).to_csv(out/'worker_costs.csv',index=False)
    ...
    content+=f'Independent validation maximum observed arithmetic/prediction discrepancy was **{validation["maximum_observed_difference"]:.8g}**; [the per-check CSV](smart_building_conformal/outputs/matched_intervals005/bdg2_pilot_v1_coordinator/validation_v2/reconstruction_checks.csv) records each frozen tolerance. All checks passed; this maximum may arise from large pooled sums rather than prediction reload.\n\n'
