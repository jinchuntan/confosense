...
def empirical_interpretation(common,contrasts):
    text='## Observed calibration and updating effects\n\n'
    for left,label in [('cqr','CQR versus its shared uncalibrated quantiles'),('recentred_enbpi_updated','Updated versus static recentred EnbPI')]:
        part=contrasts[(contrasts.support=='common')&(contrasts.left_method==left)]
        coverage=100*part.coverage_difference
        gap=part.absolute_coverage_deviation_difference
        width=part.mpiw_difference;score=part.winkler_difference
        text+=f'**{label}:** across the six horizon/level combinations, coverage changed by {coverage.min():+.3f} to {coverage.max():+.3f} percentage points. Absolute distance from nominal coverage decreased in {int((gap<0).sum())}/6 combinations. MPIW changed by {width.min():+.3f} to {width.max():+.3f} kWh, while Winkler changed by {score.min():+.3f} to {score.max():+.3f} kWh; Winkler was lower in {int((score<0).sum())}/6 combinations. These are descriptive paired differences on common support, with no independence or significance claim.\n\n'
    dscp=common[common.method=='dscp']
    text+=f'DSCP achieved {100*dscp.coverage.min():.3f}% to {100*dscp.coverage.max():.3f}% coverage across the six cells. Its interval owner uses the existing matched XGBoost predictions, so differences from the quantile and EnbPI methods include predictor differences. All reported widths and scores are evaluated on the same 34,590 common origins per horizon.\n\n'
    return text


def main(out):
    out=Path(out);out.mkdir(parents=True,exist_ok=False)
    protocol=read(DESIGN/'frozen_protocol.json');validation=read(BATCH/'validation_v5/validation.json');resume=read(BATCH/'completed_resume_v1.json')
    ...
    from check_report_inputs import check
    ...
    for action in ('run','validate','resume'):
        m=read(BATCH/(action+'_worker_resources_v5.json' if action=='validate' else action+'_worker_resources_v1.json'));assert m['actual_exit']==0
    ...
    for revision in (1,2,3,4):
        failed=read(BATCH/f'validate_worker_resources_v{revision}.json');assert failed['actual_exit']==1
        failed_measures.append(dict(action=f'validate_failed_v{revision}',actual_exit=1,started_utc=failed['started_utc'],ended_utc=failed['ended_utc'],pid=failed['pid'],**failed['resources']))
    pd.DataFrame(measures+failed_measures).to_csv(out/'worker_costs.csv',index=False)
    ...
    bootstrap=[]
    for h in (1,3,6):
        payload=read(RUN/f'stages/owner_cal_h{h}_enbpi/stage.json')['payload']
        bootstrap.append(dict(horizon=h,calibration_rows=payload['n_calibration'],nonfinite_oob_scores=payload['nonfinite_native_scores'],finite_oob_scores=payload['n_calibration']-payload['nonfinite_native_scores'],fraction_nonfinite=payload['nonfinite_native_scores']/payload['n_calibration']))
    bootstrap=pd.DataFrame(bootstrap);bootstrap.to_csv(out/'enbpi_calibration_support.csv',index=False)
    dscp_metadata=read(RUN/'stages/dscp_fit/calibrator_metadata.json')
    first_assignment=read(RUN/'stages/dscp_assignment_000000_000064/stage.json')['resources']
    atomic(out/'dscp_calibration_summary.json',dict(calibrator=dscp_metadata,first_64_assignment_resources=first_assignment,first_64_reused_in_full_cohort=True))
    ...
    atomic(out/'next_proposed_costs.json',dict(method_cells=120,triplet_units=4,new_cqr_wrappers=24,new_quantile_estimators=72,new_enbpi_wrappers=12,new_xgboost_enbpi_estimator_fits=132,new_matched_xgboost_or_lstm_fits=0,dscp_calibrators=4,kmeans_candidates=20,seasonal_new_computations=0,clean_alert_streams=120,measured_single_pilot_worker_seconds=run_measure['seconds'],fourfold_wall_scaling_seconds=4*run_measure['seconds'],fourfold_cpu_scaling_seconds=4*run_measure['process_cpu_seconds'],fourfold_validation_wall_seconds=4*measures[1]['seconds'],fourfold_resume_wall_seconds=4*measures[2]['seconds'],fourfold_run_validate_resume_wall_seconds=4*sum(m['seconds'] for m in measures),fourfold_run_validate_resume_cpu_seconds=4*sum(m['process_cpu_seconds'] for m in measures),observed_max_worker_peak_rss_bytes=max(m['peak_rss_bytes'] for m in measures),memory_scaling='sequential reuse; peak is not multiplied by four; future actual usage must be measured',planning_interpretation='linear scaling from one measured pilot, not a confidence interval or runtime guarantee; separate validation/resume overhead',authorized=False))
    ...
    crossed=native[native.raw_crossed>0]
    if len(crossed):content+='Raw bound crossings remain explicit below. Shared-owner consumers can show the same crossing rows twice; these are not distinct predictor failures. The frozen policy orders emitted bounds while retaining each raw pair.\n\n'+table(crossed,['horizon','level','method','n','raw_crossed'])+'\n'
    ...
    content+='![Measured nonoverlapping stage costs]('+prefix+'figures/stage_costs.png)\n\n'
    reconstruction=pd.read_csv(BATCH/'validation_v5/reconstruction_checks.csv')
    largest=reconstruction.sort_values('maximum_difference',ascending=False).head(10)
    largest.to_csv(out/'largest_reconstruction_differences.csv',index=False)
    worst=largest.iloc[0]
    content+=f'Independent validation maximum observed arithmetic/prediction discrepancy was **{validation["maximum_observed_difference"]:.8g}**; [the per-check CSV](smart_building_conformal/outputs/matched_intervals005/bdg2_pilot_v1_coordinator/validation_v5/reconstruction_checks.csv) records each frozen tolerance. All checks passed under their unchanged absolute/relative tolerances. The largest observed difference is from `{worst["check"]}`. [Largest discrepancies and their exact tolerances]('+prefix+'largest_reconstruction_differences.csv) distinguish point serialization, native bounds and pooled arithmetic.\n\n'
    content+='[Additional integrity checks]('+prefix+'additional_integrity_validation.json) independently reconstruct all 990 background-exposure/rate rows and 60 native/common crossing counts, reconcile every started/returned operation, and confirm base-estimator seed 42. They preserve all scientific files and perform no fitting.\n\n'
