...
def main(out):
    ...
    protocol=read(DESIGN/'frozen_protocol.json');validation=read(BATCH/'validation_v4/validation.json');resume=read(BATCH/'completed_resume_v1.json')
    ...
    for action in ('run','validate','resume'):
        m=read(BATCH/(action+'_worker_resources_v4.json' if action=='validate' else action+'_worker_resources_v1.json'));assert m['actual_exit']==0
    ...
    content+=f'Independent validation maximum observed arithmetic/prediction discrepancy was **{validation["maximum_observed_difference"]:.8g}**; [the per-check CSV](smart_building_conformal/outputs/matched_intervals005/bdg2_pilot_v1_coordinator/validation_v4/reconstruction_checks.csv) records each frozen tolerance. All checks passed; this maximum may arise from large pooled sums rather than prediction reload.\n\n'
    ...
    implementation+='The synthetic evidence contains20 two-group/two-horizon method streams,34 tiny learned-estimator fits and two DSCP calibrators, separate from real results. The initial 25-check command had one scalar-fixture failure;24 passed. Its corrected no-fit recheck and an additional native-formula/unseen-group test passed, reusing saved owners. Eleven generic coordinator tests passed. A further no-fit held-out-truth perturbation through the joint DSCP assembly also passed; its first reader attempt used the wrong saved outcome column and was corrected without touching owners or scientific code. See [supplemental acceptance](review/matched_intervals005_bdg2_20260914/SUPPLEMENTAL_ACCEPTANCE.md). Eight additional no-fit native-CQR and read-only source-identity regression checks passed after the first validation erratum; those eight plus a new native-EnbPI accumulator check passed after the second correction. The final ten-check no-fit rerun also includes a new exact float32 CSV roundtrip check. Thus **48 distinct acceptance checks are resolved**; the initial failure is retained in [the recovery record](review/matched_intervals005_bdg2_20260914/SYNTHETIC_RECOVERY.md). No tiny learned or calibrator fit was repeated for recovery.\n\n'
