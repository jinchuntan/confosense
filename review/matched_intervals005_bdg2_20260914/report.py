...
def main(out):
    ...
    protocol=read(DESIGN/'frozen_protocol.json');validation=read(BATCH/'validation_v3/validation.json');resume=read(BATCH/'completed_resume_v1.json')
    ...
    for action in ('run','validate','resume'):
        m=read(BATCH/(action+'_worker_resources_v3.json' if action=='validate' else action+'_worker_resources_v1.json'));assert m['actual_exit']==0
    ...
    content+=f'Independent validation maximum observed arithmetic/prediction discrepancy was **{validation["maximum_observed_difference"]:.8g}**; [the per-check CSV](smart_building_conformal/outputs/matched_intervals005/bdg2_pilot_v1_coordinator/validation_v3/reconstruction_checks.csv) records each frozen tolerance. All checks passed; this maximum may arise from large pooled sums rather than prediction reload.\n\n'
