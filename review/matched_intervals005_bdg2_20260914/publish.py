...
def index():
    ...
    if (REVIEW/'SUPPLEMENTAL_ACCEPTANCE.json').exists():text+='[Supplemental no-fit DSCP truth-independence acceptance](SUPPLEMENTAL_ACCEPTANCE.md).\n\n'
    if (REVIEW/'VALIDATION_ERRATUM.json').exists():text+='[Native CQR validation/specification erratum](VALIDATION_ERRATUM.md), [resolved method specification](method_specification_resolved_v2.json), [exact read-only source manifest](VALIDATION_ERRATUM.json), and [corrected validation source](corrected_validation_source.zip).\n\n'
    if (REVIEW/'VALIDATION_ERRATUM_V3.json').exists():text+='**Preceding validation correction:** [EnbPI precision erratum](VALIDATION_ERRATUM_V3.md), [revision-3 source manifest](VALIDATION_ERRATUM_V3.json), [exact recovery commands](VALIDATION_RECOVERY_COMMANDS_V3.json), and [final corrected validation source](corrected_validation_source_v3.zip). Both earlier failed validators are retained.\n\n'
    if (REVIEW/'VALIDATION_ERRATUM_V4.json').exists():text+='**Final validation version:** [native-point roundtrip erratum](VALIDATION_ERRATUM_V4.md), [revision-4 source manifest](VALIDATION_ERRATUM_V4.json), [exact recovery commands](VALIDATION_RECOVERY_COMMANDS_V4.json), and [final validation source](corrected_validation_source_v4.zip). All three earlier failed validators are retained.\n\n'
    if (REVIEW/'VALIDATION_ERRATUM_V5.json').exists():text+='**Current accepted validation source:** [cleanup import record](VALIDATION_ERRATUM_V5.md), [revision-5 source manifest](VALIDATION_ERRATUM_V5.json), [exact recovery commands](VALIDATION_RECOVERY_COMMANDS_V5.json), and [current source archive](corrected_validation_source_v5.zip). All four earlier failed validators are retained.\n\n'
    ...
    if (analysis/'COMPLETE.json').exists():
        ...
        for name,purpose in [('native_support_metrics.csv','30 native-support method cells'),('common_support_metrics.csv','30 views on the identical DSCP-supported rows'),('per_building_metrics.csv','original-building counts and metrics'),('shared_owner_contrasts.csv','CQR minus uncalibrated and updated minus static'),('background_workload.csv','numerical/availability/combined hourly immediate episodes and exposure'),('seasonal_point_metrics.csv','three deterministic point computations'),('seasonal_interval_metrics.csv','six seasonal own-residual interval cells'),('worker_costs.csv','whole CLI worker wall/CPU/memory'),('stage_costs.csv','nonoverlapping scientific stage wall/CPU/memory'),('preparation_costs.csv','full preparation costs, excluded from stage no-op bookkeeping'),('enbpi_calibration_support.csv','finite and nonfinite out-of-bag calibration score counts'),('dscp_calibration_summary.json','calibration clusters, neighbours and reused first-64 assignment cost'),('operations.csv','nested wrapper, learned estimator and calibrator calls; not additive runtime'),('interval_completion_overlay.csv','30/1950 completed method keys'),('next_proposed_method_keys.csv','120 explicit unlaunched next keys'),('next_proposed_costs.json','measured pilot scaling and sharing counts'),('analysis_validation.json','reconciled completion counts'),('background_and_crossing_checks.csv','990 independent background rate/exposure checks'),('additional_integrity_validation.json','crossings, exposures, seed and operation-journal reconciliation')]:text+=f'- [{name}]({prefix}{name}): {purpose}.\n'
