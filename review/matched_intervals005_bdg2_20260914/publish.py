...
def index():
    ...
    if (BATCH/'validation_v2/validation.json').exists():text+=f'\n[Independent validation]({b}validation_v2/validation.json), [all arithmetic/model/quantile checks]({b}validation_v2/reconstruction_checks.csv), [alert checks]({b}validation_v2/alert_checks.csv), [zero-fit/calibrator-fit completed resume]({b}completed_resume_v1.json).\n'
