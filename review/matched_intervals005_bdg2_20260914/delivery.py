...
def main():
    ...
    run=read(RUN/'COMPLETE.json');validation=read(BATCH/'validation_v2/validation.json');resume=read(BATCH/'completed_resume_v1.json');analysis=read(BATCH/'analysis_v1/analysis_validation.json')
    ...
    outer=list(BACKUP.glob('coordinator_*.log.json'));assert len(outer)==1
    outer_receipt=read(outer[0]);assert outer_receipt['exit_status']==0
    receipts=[]
    for task,record in state['tasks'].items():
        assert record['status']=='passed' and record['exit_code']==0
        receipt=read(record['logger_receipt']);assert receipt['exit_status']==0
        receipts.append(dict(task=task,**receipt))
    out=REVIEW/'delivery_v1';out.mkdir(exist_ok=False)
    copied=[]
    for pattern in ['coordinator_*.log','coordinator_*.log.json','coordinator_*.log.started.json','coordinator_*.log.launch.json','prefit_*_publication.json','prefit_remote_verification.json','completed_*_publication.json','atomic_replace_retries.jsonl','measured_cli_help.json']:
    ...
    atomic(out/'command_receipts.json',dict(coordinator=outer_receipt,phases=receipts))
    counts=read(RUN/'stages/tables/operation_counts.json')
    result=dict(passed=True,coordinator_exit=0,phase_exit_codes={r['task']:r['exit_status'] for r in receipts},method_cells=30,seasonal_point_cells=3,seasonal_interval_cells=6,alert_streams=30,actual_learned_estimator_fits=counts['quantile_estimator_fit']+counts['xgboost_estimator_fit'],operation_counts=counts,synthetic_learned_fits=34,synthetic_dscp_calibrators=2,unique_acceptance_checks=46,completed_resume_learned_fits=resume['models_fitted'],completed_resume_calibrator_fits=resume['calibrators_fitted'],scientific_files_unchanged=True,source_hash=run['source_hash'],coordinator_wall_seconds=outer_receipt['seconds'],full_study_ready=False)
    atomic(out/'validation.json',result)
    text='# Completed interval-pilot delivery\n\nThe actual coordinator and all five durable phase commands exited **0**. Independent validation passed30 method cells,6 seasonal interval cells and30 alert streams. Three seasonal point computations are separately recorded. Completed resume executed zero learned and zero calibrator fits and left all scientific run bytes unchanged.\n\n'
    text+='Real operations:6 CQR wrappers/18 HistGradientBoosting quantile fits;3 EnbPI wrappers/33 XGBoost fits;1 DSCP calibrator/5 KMeans candidates. These are51 learned estimator fits plus the separately counted clustering/calibration operations. Synthetic fits:34 tiny learned estimators and2 DSCP calibrators;46 distinct acceptance checks resolved, with the initial fixture failure retained and corrected without refitting.\n\n'
    text+=f'Actual coordinator elapsed seconds: {outer_receipt["seconds"]:.6f}. [Exact command receipts](command_receipts.json) contain PIDs, argv, timestamps and actual exits. [Whole-worker wall/CPU/memory](../../../smart_building_conformal/outputs/matched_intervals005/bdg2_pilot_v1_coordinator/analysis_v1/worker_costs.csv) and [stage costs](../../../smart_building_conformal/outputs/matched_intervals005/bdg2_pilot_v1_coordinator/analysis_v1/stage_costs.csv) must not be added to enclosing coordinator time.\n\n'
