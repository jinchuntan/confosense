...
def main():
    ...
    run=read(RUN/'COMPLETE.json');validation=read(BATCH/'validation_v3/validation.json');resume=read(BATCH/'completed_resume_v1.json');analysis=read(BATCH/'analysis_v1/analysis_validation.json')
    ...
    outer=sorted(BACKUP.glob('coordinator_*.log.json'));assert len(outer)==3
    outer_receipts=[read(path) for path in outer]
    assert [r['exit_status'] for r in outer_receipts]==[1,1,0]
    outer_receipt=outer_receipts[-1]
    receipts=[]
    for task,record in state['tasks'].items():
        expected=1 if task in ('pilot/validate','pilot/validate_v2') else 0
