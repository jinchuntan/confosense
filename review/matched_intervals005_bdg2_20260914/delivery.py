...
def main():
    ...
    run=read(RUN/'COMPLETE.json');validation=read(BATCH/'validation_v5/validation.json');resume=read(BATCH/'completed_resume_v1.json');analysis=read(BATCH/'analysis_v1/analysis_validation.json')
    ...
    outer=sorted(BACKUP.glob('coordinator_*.log.json'));assert len(outer)==5
    outer_receipts=[read(path) for path in outer]
    assert [r['exit_status'] for r in outer_receipts]==[1,1,1,1,0]
    outer_receipt=outer_receipts[-1]
    receipts=[]
    for task,record in state['tasks'].items():
        expected=1 if task in ('pilot/validate','pilot/validate_v2','pilot/validate_v3','pilot/validate_v4') else 0
