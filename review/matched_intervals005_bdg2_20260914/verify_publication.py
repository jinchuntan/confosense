...
def main():
    ...
    assert read(BATCH/'validation_v2/validation.json')['passed'] and read(BATCH/'completed_resume_v1.json')['all_run_files_unchanged']
    ...
    samples += [BATCH.relative_to(ROOT).as_posix()+'/'+name for name in ['validation_v2/validation.json','validation_v2/reconstruction_checks.csv','completed_resume_v1.json','run_worker_resources_v1.json','validate_worker_resources_v2.json','resume_worker_resources_v1.json','analysis_v1/native_support_metrics.csv','analysis_v1/common_support_metrics.csv','analysis_v1/per_building_metrics.csv','analysis_v1/background_workload.csv','analysis_v1/worker_costs.csv','analysis_v1/operations.csv','analysis_v1/seasonal_interval_metrics.csv','analysis_v1/shared_owner_contrasts.csv','analysis_v1/next_proposed_method_keys.csv']]
    for h in (1,3,6):
        for l in (90,95):
