"""Measure the entire CLI worker while retaining the existing durable logger."""
import os
...
def main():
    ...
    atomic(args.measurement,dict(started_utc=started,ended_utc=now(),pid=os.getpid(),action_argv=command,actual_exit=status,resources=meter.result,error=error,interpretation='whole Python CLI worker, excluding parent logger and interpreter startup'))
