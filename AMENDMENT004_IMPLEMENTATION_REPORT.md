# Amendment 004 implementation report
...
- Final [regression log](smart_building_conformal/outputs/amendment004/validation/regression_v3.log): **332 passed, 1 skipped, three existing MAPIE warnings; exit 0**. The skip is retained, not converted into a pass. The new runner-level test includes real checkpoint serialization, output validation and resume; the actual CQR interface has its own separate test.
...
See [per-fold event support](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/event_support.csv), [family/severity counts](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/stratum_support.csv), [exposure/memberships](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/memberships_summary.csv), [catalogue accounting](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/catalogue_summary.csv), [rank support](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/calibration_rank_support.csv) and [physical applicability](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/physical_rule_applicability.csv). The supplemental [group exposure](smart_building_conformal/outputs/amendment004/preflight_validation_20260913_v1/group_exposure.csv), [boundary-excluded origin slots](smart_building_conformal/outputs/amendment004/preflight_validation_20260913_v1/boundary_excluded_slots.csv) and [relative mask aliases](smart_building_conformal/outputs/amendment004/preflight_validation_20260913_v1/coarse_mask_aliases.csv) preserve denominators and discretization limits.
...
The no-fit command took 92.43 s; the measured preparation/audit region took 84.41 s, with peak sampled RSS **1,215,311,872 bytes**. [Per-task resources](smart_building_conformal/outputs/amendment004/preflight_20260913_v1/resources.csv) remain separate from the original pilot's timings.
...
- [Output validation](smart_building_conformal/outputs/amendment004/smoke_20260913_v2/output_validation.json): **12/12 expected inner cells**, **60/60 inner catalogue/hash pairs**, **20/20 outer diagnostic pairs**, and **96 persisted streams** rehashed after CSV reload. [Expected](smart_building_conformal/outputs/amendment004/smoke_20260913_v2/expected_inner_keys.csv) and [observed](smart_building_conformal/outputs/amendment004/smoke_20260913_v2/actual_inner_keys.csv) keys agree exactly.
...
- [Resume](smart_building_conformal/outputs/amendment004/smoke_20260913_v2/resume_validation.json) reuses **one completed unit with zero new fit invocations**. Intentional corruption of a temporary checkpoint copy is rejected. The full-study validator rejects this smoke. Seven family-specific zero controls and five zero-catalogue replays are identity checks, not performance observations.
- Compute-region time: **23.45 s**; command wall time: **37.11 s**; sampled peak RSS **267,087,872 bytes**. [Resources](smart_building_conformal/outputs/amendment004/smoke_20260913_v2/resources.csv) and [command log](smart_building_conformal/outputs/amendment004/validation/smoke_v2.log) give the exact scope and hardware. These timings are not measured BDG2 fitting costs.
...
| Identity | Value |
|---|---|
| Evaluated implementation commit | `917d6eb2d634a457f3fba0fc16133463fe05b9b4` |
| Evaluated source SHA-256 | `fd7a6bbb5fda25a7058951f49e1c41ec47f4cffe0a21640036ad8f56a7031bd8` |
| Frozen configuration SHA-256 | `b49eb9b3802e715b150c5cf3ed7681f0da85b913425c29c6460b74ef8c19a5fb` |
| Design SHA-256 | `5c1d4aedbbdedfb5d6cceedfd56b1dbe331233d0c497ea7b0126cf6095938c75` |
| Preflight source SHA-256 | `3dea8da16ae2f9520219d58d86b500dbdff7b9115e87f8ce0d4c1fc686282838` |

The [final source archive](smart_building_conformal/outputs/amendment004/validation/evaluated_source_v3.zip) and [identity](smart_building_conformal/outputs/amendment004/validation/evaluated_source_v3.json) preserve exact evaluated bytes across Git line-ending conversion. The [preflight archive](smart_building_conformal/outputs/amendment004/validation/preflight_source.zip) preserves the earlier no-fit implementation. Reporting/publication commits are later descendants, not substituted evaluated source.

The [unit directory](smart_building_conformal/outputs/amendment004/smoke_20260913_v2/units/outer0_model42) contains `surface.csv.gz`, rejection tables, catalogue allocation/events, clean/corrupted `streams.csv.gz`, `hashes.csv.gz`, episode and per-event accounting, group workload, family attribution, recovery, updates and released scores. `outer_*` files contain the separate controlled/fixed/independent comparisons. `actual_cqr_*` files hold the tiny real-interface evidence. `sufficient_*` and unsupported fixtures are explicitly synthetic; `payload.json` retains estimator identity, configuration, memberships and paired draw indices. `COMPLETE.json` hashes every unit file. No raw source dataset, environment or external backup is added to Git.

All commands ran from `smart_building_conformal/` using `C:/cfs_venv/Scripts/python.exe`, `-B`, and numerical thread variables `OMP_NUM_THREADS`, `OPENBLAS_NUM_THREADS`, `MKL_NUM_THREADS`, `NUMEXPR_NUM_THREADS` set to `1`. Each [execution record](smart_building_conformal/outputs/amendment004/validation) saves the exact argv, working directory, UTC endpoints, elapsed seconds and exit status. The source archive and existing environment are required for a source-identical training resume; saved-output validation does not require raw datasets.
